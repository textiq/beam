 ...
 * @param <T> the PipelineResult which is produced
 */
public abstract class AbsSparkRunner<T extends PipelineResult> extends PipelineRunner<T> {
    private static final Logger LOG = LoggerFactory.getLogger(AbsSparkRunner.class);
    protected final SparkPipelineOptions mOptions;
    ...
    public static void updateCacheCandidates(
        Pipeline pipeline, SparkPipelineTranslator translator,
    ...
    static class CacheVisitor extends Evaluator {
        ...
        public void doVisitTransform(TransformHierarchy.Node node) {
            ...
            // The goal is to detect the PCollections accessed more than one time, and so
            // enable cache on the underlying RDDs or DStreams.
    ...
    public static class Evaluator extends Pipeline.PipelineVisitor.Defaults {
        ...
        protected boolean shouldDefer(TransformHierarchy.Node node) {
            // if the input is not a PCollection, or it is but with non merging windows,
            // don't defer.
            ...
            if (transform instanceof Combine.PerKey) {
                List<PCollectionView<?>> sideInputs = ((Combine.PerKey<?, ?, ?>) transform)
                    .getSideInputs();
                hasSideInput = sideInputs != null && !sideInputs.isEmpty();
            } else if (transform instanceof Combine.Globally) {
                List<PCollectionView<?>> sideInputs = ((Combine.Globally<?, ?>) transform)
                    .getSideInputs();
                hasSideInput = sideInputs != null && !sideInputs.isEmpty();
            }
