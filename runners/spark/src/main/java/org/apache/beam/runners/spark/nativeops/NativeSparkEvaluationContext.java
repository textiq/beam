...
public class NativeSparkEvaluationContext extends EvaluationContext implements INativeSparkContext {
    ...
    private Map<String, JavaRDD<?>> pValueToRddMap= new HashMap<>();;
     ...
     */
    public void computeOutputs(NativeSpark nativeSparkCode) {
    ...
    public void putDataset(PValue pvalue, Dataset dataset) {
        super.putDataset(pvalue, dataset);
        if ( dataset.getType() == TranslationMode.BATCH) {
            pValueToRddMap.put(pvalue.getName(), ((BoundedDataset) dataset).getRDD());
        }
    }

    @Override
    public void putDataset(PTransform<?, ? extends PValue> transform, Dataset dataset) {
        super.putDataset(transform, dataset);
        if ( dataset.getType() == TranslationMode.BATCH) {
            pValueToRddMap.put(getOutput(transform).getName(), ((BoundedDataset)dataset).getRDD());
        }
    }
}